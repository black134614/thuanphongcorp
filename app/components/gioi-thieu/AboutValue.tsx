import React from 'react'

export default function AboutValue() {
  return (
    <div className='aboutValue main'>
      <div className='--vimision'>
        <div className='row'>
          <div className='col-md-6'>
            <div className='--left'>
              <div
                className='subTitle mb-3 aos-init aos-animate'
                data-aos='fade-up'>
                Thuận Phong Corp
              </div>
              <h2
                className='segTitle mb-3 aos-init aos-animate'
                data-aos='fade-up'
                data-aos-delay={100}>
                Sứ Mệnh &amp; Tầm Nhìn
              </h2>
              <article
                data-aos='fade-up'
                data-aos-delay={200}
                className='aos-init aos-animate'>
                Contrary to popular belief, Lorem Ipsum is not simply random
                text.
              </article>
            </div>
          </div>
          <div className='col-md-6'>
            <div className='--right'>
              <div className='--item'>
                <h6 data-aos='fade-up' className='aos-init aos-animate'>
                  Sứ mệnh
                </h6>
                <ul>
                  <li
                    data-aos='fade-up'
                    data-aos-delay={100}
                    className='aos-init aos-animate'>
                    <div className='arrow_1 d-flex align-items-center '>
                      <p className='mb-0' />
                    </div>
                    <span>
                      Contrary to popular belief, Lorem Ipsum is not simply
                      random text. It has roots in a piece of classical Latin
                      literature from 45 BC, making it over 2000 years old.
                    </span>
                  </li>
                  <li
                    data-aos='fade-up'
                    data-aos-delay={200}
                    className='aos-init aos-animate'>
                    <div className='arrow_1 d-flex align-items-center '>
                      <p className='mb-0' />
                    </div>
                    <span>
                      Contrary to popular belief, Lorem Ipsum is not simply
                      random text. It has roots in a piece of classical Latin
                      literature from 45 BC, making it over 2000 years old.
                    </span>
                  </li>
                  <li
                    data-aos='fade-up'
                    data-aos-delay={300}
                    className='aos-init aos-animate'>
                    <div className='arrow_1 d-flex align-items-center '>
                      <p className='mb-0' />
                    </div>
                    <span>
                      Contrary to popular belief, Lorem Ipsum is not simply
                      random text. It has roots in a piece of classical Latin
                      literature from 45 BC, making it over 2000 years old.
                    </span>
                  </li>
                </ul>
              </div>
              <div className='--item'>
                <h6 data-aos='fade-up' className='aos-init aos-animate'>
                  Tầm nhìn
                </h6>
                <ul>
                  <li
                    data-aos='fade-up'
                    data-aos-delay={100}
                    className='aos-init aos-animate'>
                    <div className='arrow_1 d-flex align-items-center '>
                      <p className='mb-0' />
                    </div>
                    <span>
                      Contrary to popular belief, Lorem Ipsum is not simply
                      random text. It has roots in a piece of classical Latin
                      literature from 45 BC, making it over 2000 years old.
                    </span>
                  </li>
                  <li
                    data-aos='fade-up'
                    data-aos-delay={200}
                    className='aos-init aos-animate'>
                    <div className='arrow_1 d-flex align-items-center '>
                      <p className='mb-0' />
                    </div>
                    <span>
                      Contrary to popular belief, Lorem Ipsum is not simply
                      random text. It has roots in a piece of classical Latin
                      literature from 45 BC, making it over 2000 years old.
                    </span>
                  </li>
                  <li
                    data-aos='fade-up'
                    data-aos-delay={300}
                    className='aos-init aos-animate'>
                    <div className='arrow_1 d-flex align-items-center '>
                      <p className='mb-0' />
                    </div>
                    <span>
                      Contrary to popular belief, Lorem Ipsum is not simply
                      random text. It has roots in a piece of classical Latin
                      literature from 45 BC, making it over 2000 years old.
                    </span>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className='--corevalue'>
        <h6 data-aos='fade-up' className='aos-init aos-animate'>
          Giá trị cốt lõi
        </h6>
        <div className='list_corevalue d-flex justify-content-between'>
          <div
            className='--item aos-init aos-animate'
            data-aos='fade-up'
            data-aos-delay={200}>
            <div className='--txt'>
              <div className='arrow_1 d-flex align-items-center '>
                <p className='mb-0' />
              </div>
              <h3>“Dịch vụ uy tín, chuyên nghiệp, tiến độ nhanh”</h3>
              <span>
                Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of de
                Finibus Bonorum et Malorum (The Extremes of Good and Evil) by
                Cicero, written in 45 BC. This book is a treatise on the theory
                of ethics, very popular during the Renaissance. The first line
                of Lorem Ipsum, Lorem ipsum dolor sit amet.., comes from a
                line in section 1.10.32.
              </span>
              <div className='--link mt-3 justify-content-center --icon'>
                <img src='/assets/imgs/Arrow 8.svg' alt='SUN GROUP' />
              </div>
            </div>
            <div className='partent'>
              <img src='/assets/imgs/partent_2.png' alt='SUN GROUP' />
            </div>
          </div>
          <div
            className='--item aos-init aos-animate'
            data-aos='fade-up'
            data-aos-delay={400}>
            <div className='--txt'>
              <div className='arrow_1 d-flex align-items-center '>
                <p className='mb-0' />
              </div>
              <h3>“Môi trường làm việc sáng tạo, hiệu quả”</h3>
              <span>
                Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of de
                Finibus Bonorum et Malorum (The Extremes of Good and Evil) by
                Cicero, written in 45 BC. This book is a treatise on the theory
                of ethics, very popular during the Renaissance. The first line
                of Lorem Ipsum, Lorem ipsum dolor sit amet.., comes from a
                line in section 1.10.32.
              </span>
              <div className='--link mt-3 justify-content-center --icon'>
                <img src='/assets/imgs/Arrow 8.svg' alt='SUN GROUP' />
              </div>
            </div>
            <div className='partent'>
              <img src='/assets/imgs/partent_2.png' alt='SUN GROUP' />
            </div>
          </div>
          <div
            className='--item aos-init aos-animate'
            data-aos='fade-up'
            data-aos-delay={600}>
            <div className='--txt'>
              <div className='arrow_1 d-flex align-items-center '>
                <p className='mb-0' />
              </div>
              <h3>“Đội ngũ nhân viên đoàn kết và thân thiện”</h3>
              <span>
                Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of de
                Finibus Bonorum et Malorum (The Extremes of Good and Evil) by
                Cicero, written in 45 BC. This book is a treatise on the theory
                of ethics, very popular during the Renaissance. The first line
                of Lorem Ipsum, Lorem ipsum dolor sit amet.., comes from a
                line in section 1.10.32.
              </span>
              <div className='--link mt-3 justify-content-center --icon'>
                <img src='/assets/imgs/Arrow 8.svg' alt='SUN GROUP' />
              </div>
            </div>
            <div className='partent'>
              <img src='/assets/imgs/partent_2.png' alt='SUN GROUP' />
            </div>
          </div>
        </div>
      </div>
      <div className='--vimision mobi d-none'>
        <div className='--left'>
          <div
            className='subTitle mb-3 aos-init aos-animate'
            data-aos='fade-up'>
            Thuận Phong Corp
          </div>
          <h2
            className='segTitle mb-3 aos-init aos-animate'
            data-aos='fade-up'
            data-aos-delay={100}>
            Sứ Mệnh &amp; Tầm Nhìn
          </h2>
          <article
            data-aos='fade-up'
            data-aos-delay={200}
            className='aos-init aos-animate'>
            Contrary to popular belief, Lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC
          </article>
        </div>
      </div>
      <div className='--img'>
        <img
          src='https://sun-ecommerce-cdn.azureedge.net/ecommerce/service-sites/thumbnail/SunGroup/_default_upload_bucket/1909/image-thumb__1909__image-thumbnail/img-2_1664951238.png'
          alt='SUN GROUP'
        />
      </div>
    </div>
  )
}
